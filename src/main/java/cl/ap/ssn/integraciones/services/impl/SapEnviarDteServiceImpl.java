 ...
 *
 */
@Service("enviarDteService")
public class SapEnviarDteServiceImpl extends AbstractProcessService<IntegracionSapEnvioDteDTO> {
	...
	public SapEnviarDteServiceImpl(EmailService emailService,
			SapClient sapClient,
		...
		this.proceso = "ActualizarSAP";
	}

	@Override
	public void procesoIntegracion(final IntegracionSapEnvioDteDTO integracion) {
		...
		try {
			...
			if(pendiente.isEmpty()) {
				log.warn(String.format("[%s] Sin DTE pendiente", integracion.toStringSuper()));
				return;
			}
			/*
			 * Actualizar estado
			...
			if(!this.manejoMensajeDatabase(log, integracion, resultadoProc, respuestaSap.getBody(),
				...
				log.info(String.format("Problema en proceso DTE %s", apiInfo.get().getEndpoint()));
	...
	private Optional<SapApiCallDTO> obtenerApiInfo(final IntegracionSapEnvioDteDTO integracion) {
		/*
