 ...
 *
 */
@Service("generarNcService")
public class SapGenerarNcServiceImpl extends AbstractProcessService<IntegracionSapGenerarNcDTO> {
	...
	public SapGenerarNcServiceImpl(EmailService emailService,
			SapClient sapClient,
		...
		this.proceso = "GenerarSAPNotaCredito";
	}

	@Override
	public void procesoIntegracion(final IntegracionSapGenerarNcDTO integracion) {
		...
		try {
			...
			if(pendiente.isEmpty()) {
				log.warn(String.format("[%s] Sin NC pendientes", integracion.toStringSuper()));
				return;
			}
			/*
			...
			if(!this.manejoMensajeDatabase(log, integracion, resultadoProc, respuestaSap.getBody(),
				...
				this.actualizarLogIo(integracion, logIoTmp, "ERROR", resultadoProc.get().getEstado());
				return;
			}
			/*
			 * Procesar orden SAP
			 */
			final Optional<String> prcSap = this.procesarRespuestaSapNc(integracion, logIoTmp.getIdInternoOrden(),
					resultadoProc.get().getValor());
			if(prcSap.isPresent()) {
				log.info("[{}] Proceso respuesta SAP NC: {}", integracion.toStringSuper(), prcSap.get());
			}
			/*
	...
	private Optional<SapApiCallDTO> obtenerApiInfo(final IntegracionSapGenerarNcDTO integracion) {
		/*
